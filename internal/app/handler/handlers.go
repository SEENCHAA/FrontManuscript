...
// @Description Возвращает логин и роль пользователя на основе JWT-токена.
// @Tags Users
// @Produce json
// @Security ApiKeyAuth
// @Success 200 {object} ds.User "Успешный ответ с данными пользователя"
// @Failure 401 {object} ErrorResponse "Неавторизован"
// @Failure 500 {object} ErrorResponse "Ошибка сервера"
// @Router /users/me [get]
func (h *ApplicationController) ApiGetMe(c *gin.Context) {
	// Получаем ID пользователя из контекста, который установил middleware
	userIDValue, exists := c.Get("user_id")
	if !exists {
		c.JSON(http.StatusUnauthorized, ErrorResponse{"User is not authenticated"})
		return
	}
	...
	// Получаем полные данные пользователя из репозитория
	...
	// ОЧЕНЬ ВАЖНО: Обнулить пароль перед отправкой на фронтенд
	user.Password = ""
...
// @Failure 401 {object} ErrorResponse "Неавторизован"
// @Failure 500 {object} ErrorResponse "Ошибка сервера"
// @Router /users/me [get] // NOTE: Лучше перевести маршрут на /users/me
func (h *ApplicationController) ApiGetUser(c *gin.Context) {
	// --- ИСПРАВЛЕНИЕ: Используем ID из контекста, игнорируя URL-параметр ---

	// 1. Получаем ID пользователя из контекста (установлен middleware)
	userIDValue, exists := c.Get("user_id")
	if !exists {
		// Должно быть перехвачено middleware, но для безопасности
	...
	// 2. Получаем полные данные пользователя из репозитория
	user, err := h.ApplicationModel.GetUser(userID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to fetch user data: " + err.Error()})
		return
	...
	// 3. ОЧЕНЬ ВАЖНО: Обнулить пароль перед отправкой на фронтенд
	user.Password = ""
...
func (h *ApplicationController) ApiUpdateUser(c *gin.Context) {
	// 1. Получаем ID пользователя, которого нужно обновить, из URL
	idStr := c.Param("id")
	...
	// 2. Обновляем пользователя с указанным ID
	if err := h.ApplicationModel.UpdateUser(uint(targetID), data.Username, data.Password); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *ApplicationController) ApiAddLetterToDraft(c *gin.Context) {
	...
	// NOTE: Если вы передаете количество 1 в теле, то оставьте эту часть.
	// Если фронтенд вообще не отправляет тело, эту часть (BindJSON) нужно убрать,
	// а quantity захардкодить как 1. Я оставляю ваш вариант с BindJSON.
	var body UpdateQuantityRequest
	if err := c.BindJSON(&body); err != nil {
		// Если тело не пришло, предполагаем, что добавляется 1
		body.Quantity = 1
	}
	...
	// Используем ПРАВИЛЬНЫЙ ID и ожидаем 3 возвращаемых значения
	draftID, letterCount, err := h.ApplicationModel.AddLetterToDraft(currentUserID, uint(lid), body.Quantity)
	if err != nil {
		// Здесь можно добавить более детальную обработку ошибок, если нужно
	...
	// Возвращаем ID черновика и новое количество для обновления значка на фронтенде
	c.JSON(http.StatusCreated, gin.H{
		"status":        "letter added to draft",
		"letter_id":     lid,
		"manuscript_id": draftID,     // ID созданного/найденного черновика
		"letter_count":  letterCount, // Общее количество позиций в корзине
	})
}
...
func (h *ApplicationController) ApiGetBasketStatus(c *gin.Context) {
	// 1. Получаем ID текущего пользователя из контекста (установлен в middleware)
	userIDValue, exists := c.Get("user_id")
	if !exists {
	...
	// 2. Ищем корзину именно для ЭТОГО пользователя
	draftID, letterCount, err := h.ApplicationModel.GetBasketStatus(userID)
	if err != nil {
		// 404 здесь — это нормальная ситуация, если корзина пуста (еще не создана)
...
func (h *ApplicationController) ApiGetManuscripts(c *gin.Context) {
	...
	for _, m := range manuscripts {
		...
		// 5.2 Определение имени модератора
		...
		responseList = append(responseList, ds.ManuscriptListItemResponse{
			...
			ManuscriptText:   m.ManuscriptText,
			Letters:          letterList,

			// === ДОБАВЛЕНО: ЗАПОЛНЕНИЕ ДАТ ===
...
func (h *ApplicationController) ApiModerateManuscript(c *gin.Context) {
	...
	// 1. Получаем ID текущего пользователя (модератора) из токена
	userIDValue, exists := c.Get("user_id")
	if !exists {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "unauthorized"})
		return
	}
	moderatorID := userIDValue.(uint)

	var err error
	// 2. Выполняем действие с привязкой к ID модератора
	switch data.Status {
	case "finished":
